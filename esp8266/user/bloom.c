...
static unsigned int ICACHE_FLASH_ATTR bloom_hash(struct bloom *bf, unsigned int nHashNum,
				   struct const_buffer *vDataToHash_)
{
	// The following is MurmurHash3 (x86_32), see http://code.google.com/p/smhasher/source/browse/trunk/MurmurHash3.cpp
	//uint32_t h1 = nHashNum * (0xffffffffU / (bf->nHashFuncs-1));
	uint32_t h1 = nHashNum * 0xFBA4C795L + bf->nTweak;
...
static void ICACHE_FLASH_ATTR string_resize(cstring *s, unsigned int new_index)
{
...
void ICACHE_FLASH_ATTR bloom_insert(struct bloom *bf, const void *data, size_t data_len)
{
...
//bool ICACHE_FLASH_ATTR bloom_contains(struct bloom *bf, const void *data, size_t data_len)
//{
//	struct const_buffer vKey = { data, data_len };
//	unsigned int i;
//	for (i = 0; i < bf->nHashFuncs; i++)
...
bool ICACHE_FLASH_ATTR bloom_init(struct bloom *bf, unsigned int nElements, double nFPRate, unsigned int nTweak)
{
...
void ICACHE_FLASH_ATTR bloom_free(struct bloom *bf)
{
	if (bf->vData) {
