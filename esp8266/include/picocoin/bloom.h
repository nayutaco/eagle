...
extern bool ICACHE_FLASH_ATTR bloom_init(struct bloom *bf, unsigned int nElements,double nFPRate, unsigned int nTweak);
//extern void __bloom_init(struct bloom *bf);
extern void ICACHE_FLASH_ATTR bloom_free(struct bloom *bf);

//extern bool deser_bloom(struct bloom *bf, struct const_buffer *buf);
//extern void ser_bloom(cstring *s, const struct bloom *bf);

extern void ICACHE_FLASH_ATTR bloom_insert(struct bloom *bf, const void *data, size_t data_len);
